...
//v is not valid - any of these words can be rearranged to form any other word.

[<Test>]
let ``abcde fghij``()=
  "abcde fghij" |> isValid |> should equal true

[<Test>]
let ``abcde xyz ecdab``()=
  "abcde xyz ecdab" |> isValid |> should equal false

[<Test>]
let ``a ab abc abd abf abj``()=
  "a ab abc abd abf abj" |> isValid |> should equal true

[<Test>]
let ``iiii oiii ooii oooi oooo``()=
  "iiii oiii ooii oooi oooo" |> isValid |> should equal true

[<Test>]
let ``oiii ioii iioi iiio`` ()=
  "oiii ioii iioi iiio" |> isValid |> should equal false
